 ...
 * Uses XPath via query engine (Puppeteer v22+ compatible).
 */
async function waitForSignInButton(page: Page, timeoutMs = 15_000) {
  const selector = "xpath=//button[normalize-space() = 'Sign In']";
...
async function clickSignInAndLogin(
  ...
  const signInButton = await waitForSignInButton(page);
...
export async function getAuthorizationCookies(
  ...
  try {
    ...
    try {
      ...
      const cookies = await getMelCloudHomeSecureCookies(page);
